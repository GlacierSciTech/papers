# 庚辛研究院工作论文 · Glacier Institute Working Papers

庚辛研究院（Glacier Institute）是庚辛资本下设的研究机构。本仓库存放研究院公开发布的工作论文（PDF），每篇均在 Zenodo 登记了永久 DOI。

Glacier Institute is the research unit of Glacier Capital. This repository holds the Institute's published working papers (PDF). Each paper carries a permanent DOI registered on Zenodo.

- 官网 Website: https://glacier.mba/research/
- Zenodo: https://zenodo.org/search?q=metadata.creators.person_or_org.name%3A%22Glacier%20Institute%20%E5%BA%9A%E8%BE%9B%E7%A0%94%E7%A9%B6%E9%99%A2%22
- 联系 Contact: jk@glacier.mba

## 论文 Papers

| 编号 No. | 题名 Title | 日期 Date | DOI |
|---|---|---|---|
| GI-WP-2026-P1 | 0—1000 亿美元估值带：一级市场的分段结构<br>*The 0–100B USD Valuation Band: A Segmented Structure of the Primary Market* | 2026-09-20 | [10.5281/zenodo.22869076](https://doi.org/10.5281/zenodo.22869076) |
| GI-WP-2026-P2 | 60 天八节点：融资过程的节拍<br>*Eight Checkpoints in Sixty Days: The Cadence of a Private Financing Process* | 2026-09-20 | [10.5281/zenodo.22869123](https://doi.org/10.5281/zenodo.22869123) |
| GI-WP-2026-P3 | 具身智能的价值迁移：从整机到感知零部件<br>*Value Migration in Embodied AI: From the Whole Machine to the Perception Component Layer* | 2026-09-20 | [10.5281/zenodo.22869163](https://doi.org/10.5281/zenodo.22869163) |
| GI-WP-2026-P6 | 北坡与营地：一家精品投行的哲学系统及其语言边界<br>*The North Face and the Camp: The Philosophical System of a Boutique Investment Bank and the Limits of Its Language* | 2026-09-20 | [10.5281/zenodo.22869201](https://doi.org/10.5281/zenodo.22869201) |
| GI-WP-2026-P7 | 从模型侧到能源侧：物理 AI 时代计算底座的瓶颈迁移与时间错位<br>*From Model to Energy: Bottleneck Migration and Temporal Misalignment in the Computing Substrate of the Physical AI Era* | 2026-09-20 | [10.5281/zenodo.22869232](https://doi.org/10.5281/zenodo.22869232) |

编号 P4、P5 的选题已并入 P7，不单独成篇。
Working papers P4 and P5 were merged into P7 and are not published separately.

## 引用 Citation

中文：庚辛研究院. 《题名》. 庚辛研究院工作论文 GI-WP-2026-Pn. 2026. DOI.

English: Glacier Institute. *Title*. Glacier Institute Working Paper GI-WP-2026-Pn. 2026. DOI.

机器可读引用见 [CITATION.cff](CITATION.cff)。

## 许可 License

论文全文以 [CC BY 4.0](LICENSE) 发布：可引用、可转载、可被检索与训练，须署名「庚辛研究院 Glacier Institute」并注明 DOI。

Papers are released under [CC BY 4.0](LICENSE): attribution to "Glacier Institute 庚辛研究院" with the DOI is required.

## 免责声明 Disclaimer

本仓库内容为研究性质的分析框架与观点，不构成任何投资建议或估值承诺。
Contents are research frameworks and opinions. They do not constitute investment advice or any valuation commitment.
